...
export class SvgIconComponent implements OnInit, OnDestroy, OnChanges, DoCheck {
  ...
  ngOnChanges(changeRecord: SimpleChanges) {
    ...
    if (changeRecord['src'] || changeRecord['name']) {
      if (this.helper.loaded) this.destroy();
      this.init();
    }
    if (changeRecord['stretch']) this.stylize();

    if (changeRecord['applyClass']) {
      if (this.applyClass) this.setClass(elemSvg, null, this.klass);
      else this.setClass(elemSvg, this.klass, null);
    }

    if (changeRecord['svgClass']) this.setClass(elemSvg, changeRecord['svgClass'].previousValue, changeRecord['svgClass'].currentValue);

    if (changeRecord['klass']) {
      const elem = this.element.nativeElement;
      this.setClass(elem, changeRecord['klass'].previousValue, changeRecord['klass'].currentValue);

      if (this.applyClass) this.setClass(elemSvg, changeRecord['klass'].previousValue, changeRecord['klass'].currentValue);
      else this.setClass(elemSvg, changeRecord['klass'].previousValue, null);
    }
    ...
    if (changeRecord['svgAriaLabel']) this.doAria(changeRecord['svgAriaLabel'].currentValue);
  }
  ...
  private init() {
    if (this.name) {
      ...
      if (svgObs) this.helper.icnSub = svgObs.subscribe(svg => this.initSvg(svg));
    } else if (this.src) {
  ...
  private setSvg(svg: SVGElement) {
    if (!this.helper.loaded && svg) {
      ...
      if (!(this.svgAriaLabel === undefined && elem.firstChild.hasAttribute('aria-label'))) this.doAria(this.svgAriaLabel || '');
  ...
  private copyNgContentAttribute(hostElem: any, icon: SVGElement) {
    ...
    for (let i = 0; i < len; i += 1) {
      ...
      if (attribute && attribute.name.startsWith('_ngcontent')) { this.setNgContentAttribute(icon, attribute.name); break; }
    }
  }
  ...
  private setNgContentAttribute(parent: Node, attributeName: string) {
    ...
    for (let i = 0; i < len; i += 1) {
      ...
      if (child instanceof Element) this.setNgContentAttribute(child, attributeName);
    }
  }
